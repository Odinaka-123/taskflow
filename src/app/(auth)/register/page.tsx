    ...
    <div className="min-h-screen bg-white flex">
      ...
      <div className="hidden lg:flex lg:w-[45%] bg-slate-900 flex-col justify-between p-12 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-4">
          <p className="text-slate-400 text-sm font-medium uppercase tracking-widest">Get started in 2 steps</p>
          {[
            { n: "01", title: "Your details",  desc: "Name, email, and a strong password" },
            { n: "02", title: "Choose a role", desc: "Set your default access level"       },
          ].map((s, i) => (
            <div key={s.n} className={`flex gap-4 p-4 rounded-2xl border transition-all ${
              (step === 1 && i === 0) || (step === 2 && i === 1)
                ? "bg-white/8 border-white/15"
                : "border-transparent opacity-50"
            }`}>
              <span className="text-2xl font-bold text-violet-400 leading-none">{s.n}</span>
              <div>
        ...
        <div className="relative z-10 bg-white/5 border border-white/10 rounded-2xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-white text-sm font-medium">Free forever on the Spark plan</p>
              <p className="text-slate-400 text-xs mt-0.5">No credit card required. Full access to core features.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-16 bg-white">
        <div className="w-full max-w-sm">
          ...
          <div className="flex items-center gap-2 mb-8 lg:hidden">
            <div className={`h-1 rounded-full flex-1 transition-all ${step >= 1 ? "bg-violet-500" : "bg-slate-200"}`}/>
            <div className={`h-1 rounded-full flex-1 transition-all ${step >= 2 ? "bg-violet-500" : "bg-slate-200"}`}/>
          </div>
          ...
          {step === 2 && (
            <>
              <div className="mb-8">
                <button onClick={() => setStep(1)} className="flex items-center gap-1.5 text-slate-500 hover:text-slate-700 text-sm mb-4 transition-colors">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7"/>
                  </svg>
                  Back
                </button>
                <h1 className="text-2xl font-bold text-slate-900">Pick your role</h1>
                <p className="text-slate-500 text-sm mt-1">You can change this later in settings</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                {ROLES.map(r => (
                  <button key={r.value} type="button" onClick={() => setRole(r.value)}
                    ...
                    <div className={`w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-all ${
                      role === r.value ? "border-violet-500 bg-violet-500" : "border-slate-300"
                    }`}>
                      {role === r.value && <div className="w-1.5 h-1.5 rounded-full bg-white"/>}
                    </div>
                    <div>
                      <p className={`text-sm font-semibold ${role === r.value ? "text-violet-700" : "text-slate-800"}`}>{r.label}</p>
                ...
                <button type="submit" disabled={loading}
                  className="w-full bg-violet-600 hover:bg-violet-700 active:bg-violet-800 disabled:opacity-50 text-white font-semibold rounded-xl py-2.5 text-sm transition-colors flex items-center justify-center gap-2 mt-2">
                  {loading
